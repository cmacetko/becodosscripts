...
import { generateTermsAndConditions } from "@/lib/generators";
import { showSuccess, showError } from "@/utils/toast";
import { Copy } from "lucide-react";
import { Textarea } from "@/components/ui/textarea";

export const TermsAndConditionsGenerator = () => {
  ...
  const handleGenerate = () => {
    ...
    const newText = generateTermsAndConditions({ siteName, siteUrl, format });
  ...
  const handleCopy = () => {
    if (generatedText) {
      ...
      showSuccess(`Termos e Condições (${format.toUpperCase()}) copiado para a área de transferência!`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Button onClick={handleGenerate} className="w-full sm:w-auto">Gerar Termos</Button>
